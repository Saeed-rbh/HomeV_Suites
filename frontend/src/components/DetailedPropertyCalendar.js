            ...
            {(() => {
              ...
              return weekEvents.map((ev, evIdx) => {
                ...
                return (
                  <div 
                    key={`${ev.id}-${weekIdx}`}
                    className={`absolute z-20 flex items-center h-8 bg-indigo-50 border-y border-indigo-200/80 shadow-[0_2px_4px_rgba(0,0,0,0.02)]
                  ...
                  >
                    {!isContinuingFromPrev && (
                       <div className="bg-[#FF5A5F] text-white rounded-full w-5 h-5 flex items-center justify-center shrink-0 ml-1.5 shadow-sm">
                         <AirbnbIcon />
                       </div>
                    )}
